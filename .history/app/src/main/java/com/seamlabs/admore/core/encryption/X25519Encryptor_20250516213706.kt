...
class X25519Encryptor @Inject constructor(
    ...
    override fun encrypt(data: Map<String, Any>): String {
        ...
        val encryptionKey = keyStoreManager.getEncryptionKey() 
            ?: throw IllegalStateException("Encryption key not found. Please store a key first.")
        ...
        // Create AES key from shared secret and stored encryption key
        val combinedKey = (sharedSecret + encryptionKey.toByteArray(Charsets.UTF_8))
        val derivedKey = SecretKeySpec(combinedKey, 0, 32, "AES")
