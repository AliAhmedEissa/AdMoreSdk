...
class BluetoothCollector @Inject constructor(
    ...
    override suspend fun collect(): Map<String, Any> {
        if (!isPermissionGranted() || !timeManager.shouldCollectBluetooth()) {
            return emptyMap()
        }
        ...
        if (hasScanPermission()) {
            ...
            val scanResults = withTimeoutOrNull(10000) { // 10 second timeout
        ...
        if (data.containsKey(BluetoothKeys.NEARBY_DEVICES.toKey())) {
            timeManager.updateBluetoothCollectionTime()
        }
