...
class EventRepositoryImpl @Inject constructor(
    ...
    override suspend fun sendEvent(eventName: String, eventData: Map<String, Any>) {
        // Wrap event data in array
        val eventDataArray = listOf(eventData)
        val event = Event(eventName, eventDataArray)
        
        if (networkMonitor.isNetworkAvailable()) {
            try {
                // Encrypt data before sending
                val encryptedData = dataEncryptor.encrypt(eventDataArray)
