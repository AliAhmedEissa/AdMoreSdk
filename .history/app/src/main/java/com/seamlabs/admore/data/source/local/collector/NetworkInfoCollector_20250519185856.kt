...
class NetworkInfoCollector @Inject constructor(
    ...
    override suspend fun collect(): Map<String, Any> {
        ...
        // Get cellular network details if available
        if (data[NetworkKeys.CONNECTION_TYPE.toKey()] == "cellular") {
    ...
    private fun getNetworkInfo(connectivityManager: ConnectivityManager, network: Network): Map<String, Any> {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            return mapOf(
                ...
                NetworkKeys.FAILOVER.toKey() to (networkInfo.failover ?: false),
                NetworkKeys.IS_BLOCKED.toKey() to (networkInfo.isBlocked ?: false)
            )
        }
        return emptyMap()
    }
