...
class DeviceInfoCollector @Inject constructor(
    ...
    private fun getSystemFeatures(): Map<String, Any> {
        ...
        importantFeatures.forEach { feature ->
            val featureName = feature.substring(feature.lastIndexOf('.') + 1).uppercase()
            features[DeviceInfoKeys.valueOf("FEATURE_$featureName").toKey()] = packageManager.hasSystemFeature(feature)
        }
