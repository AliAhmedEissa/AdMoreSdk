...
std::string KeyStore::KEY_ENCRYPTION_KEY;

bool KeyStore::initialize(const std::string& encryptionKey) {
    if (encryptionKey.empty()) {
        LOGE("Encryption key cannot be empty");
        return false;
    }
...
std::string KeyStore::encryptKey(const std::string& key) {
    try {
        ...
        if (EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, 
            reinterpret_cast<const unsigned char*>(KEY_ENCRYPTION_KEY.c_str()), iv) != 1) {
            EVP_CIPHER_CTX_free(ctx);
